...
std::pair<bool, size_t> SerialFileTransfer::transfer(const std::string &file_path, const std::string &file_name)
{
    ...
    try
    {
        ...
        log_info("SerialFileTransfer({})成功传输文件({}:{} bytes)", m_portname, file_complete_path, file_size);
    }
    catch(const std::exception& e)
    {
        log_error("SerialFileTransfer({})传输文件file({})失败, error msg = {}", m_portname, file_complete_path, e.what());
        ret = false;
    }
...
std::pair<bool, size_t> SerialFileTransfer::recvive(const std::string &file_path, const std::string &prefix, const std::string &suffix)
{
